export class DHCPRelayIndirectEstablishController {
  static getDI() {
    return [
      '$scope',
      '$rootScope',
  ...
  constructor(...args) {
    this.di = {};
    DHCPRelayIndirectEstablishController.getDI().forEach((value, index) => {
    ...
    scope.title = this.translate("MODULES.DHCP_RELAY.DHCPRELAY_INDIRECT.WIZARD");
    scope.steps = [
      {
        id: 'step1',
        title: this.translate('MODULES.DHCP_RELAY.DHCPRELAY_INDIRECT.WIZARD'),
        content: require('../template/dhcprelay_indirect_establish'),
      }
    ...
    scope.submit = function () {
      ...
      di.$rootScope.$emit('page_dhcprelay_indirect_es');
      if (!validCurrentDom('dhcprelay_indirect_establish')) {
        return new Promise((resolve, reject) => {
    ...
    unsubscribes.push(this.di.$rootScope.$on('relay-indirect-wizard-show', ($event, data) => {
      scope.open();
    }));
...
DHCPRelayIndirectEstablishController.$inject = DHCPRelayIndirectEstablishController.getDI();
DHCPRelayIndirectEstablishController.$$ngIsClass = true;